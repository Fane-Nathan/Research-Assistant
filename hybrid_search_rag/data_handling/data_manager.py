# -*- coding: utf-8 -*-
"""
Data Manager for StudyAssistant Project
...
class DataManager:
    ...
    def __init__(self, data_dir: str, metadata_filename: str, embeddings_filename: str, bm25_filename: str):
        ...
        Args:
            ...
            embeddings_filename: Name of the embeddings numpy file
            bm25_filename: Name of the BM25 index pickle file
        """
    ...
    def save_all_data(self, metadata: List[Dict[str, Any]], embeddings: Optional[np.ndarray], bm25_index: Optional[object]):
        ...
        Args:
            ...
            embeddings: Document embeddings array (optional)
            bm25_index: BM25 index object (optional)
            
        Raises:
            ValueError: If metadata and embeddings counts don't match
    ...
    def load_all_data(self) -> Tuple[Optional[List[Dict[str, Any]]], Optional[np.ndarray], Optional[object]]:
        ...
        Returns:
            Tuple of (metadata, embeddings, bm25_index)
            Any component can be None if loading fails or file doesn't exist
        """
                ...
                if self.metadata_path.endswith('.jsonl'):
                    for line_number, line in enumerate(f, 1):
                        try:
                            ...
                            logger.error(f"Error decoding JSON on line {line_number} in {self.metadata_path}: {e}")
                            continue 
                else: # Assume it's a standard JSON file
                    metadata_content = json.load(f)
            
            if not isinstance(metadata_content, list):
                logger.error("Metadata file format error: Expected a JSON list or JSON Lines content.")
                return None, None, None
    ...
    def get_data_summary(self) -> Dict[str, Any]:
        """
        Provides a summary of the loaded data for inspection or debugging.

        Returns:
            Dictionary containing counts of metadata items, embeddings shape, and BM25 index info.
        """
        metadata, embeddings, bm25_index = self.load_all_data()
        metadata_count = len(metadata) if metadata is not None else 0
        embeddings_shape = embeddings.shape if embeddings is not None else "No embeddings loaded"
        bm25_info = str(bm25_index) if bm25_index is not None else "No BM25 index loaded"

        return {
            "metadata_count": metadata_count,
            "embeddings_shape": embeddings_shape,
            "bm25_index_info": bm25_info,
        }
